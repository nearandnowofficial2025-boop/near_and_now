      ...
      items: [
        {
          ...
          submenu: [
            { title: 'All Products', path: '/admin/products', icon: <Package size={14} /> },
            { title: 'Add Product', path: '/admin/products/add', icon: <Plus size={14} /> },
            { title: 'Categories', path: '/admin/categories', icon: <Layers size={14} /> }
          ]
        }
      ]
    },
      ...
      >
        ...
        <div className={`flex items-center h-16 border-b border-white/10 flex-shrink-0 ${isOpen ? 'px-5 justify-between' : 'px-0 justify-center'}`}>
          {isOpen ? (
            ...
            <Link to="/admin" className="w-9 h-9 bg-emerald-500 rounded-lg flex items-center justify-center">
              <img src="/Logo.png" alt="" className="w-5 h-5 object-contain" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
            </Link>
          )}
        </div>
